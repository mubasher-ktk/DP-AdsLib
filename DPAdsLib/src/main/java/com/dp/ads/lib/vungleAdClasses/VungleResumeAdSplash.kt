package com.dp.ads.lib.vungleAdClasses
...
class VungleResumeAdSplash(
    ...
    private val timeoutRunnable = Runnable {
        if (interstitialAd == null) {
            onAdTimeout?.invoke()
            dismissWaitDialog()
            Log.i("DP_ADS_TAG", "Vungle: ResumeAd : Timeout()")
            currentActivity?.let {
                if (BuildConfig.DEBUG) {
                    Toast.makeText(it, "Vungle OpenAd Timeout", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
        ...
        interstitialAd = InterstitialAd(currentActivity!!, placementId, AdConfig()).apply {
            adListener = object : BaseAdListener {
                override fun onAdLoaded(baseAd: BaseAd) {
                    Log.i("DP_ADS_TAG", "Vungle: ResumeAd : onAdLoaded()")
                    showAdIfAvailable()
                }

                override fun onAdStart(baseAd: BaseAd) {
                    Log.i("DP_ADS_TAG", "Vungle: ResumeAd : onAdStart()")
                }

                override fun onAdEnd(baseAd: BaseAd) {
                    Log.i("DP_ADS_TAG", "Vungle: ResumeAd : onAdEnd()")
                    dismissWaitDialog()
                    isShowingAd = false
                    onAdDismissed?.invoke()
                }

                override fun onAdFailedToLoad(baseAd: BaseAd, adError: com.vungle.ads.VungleError) {
                    Log.i("DP_ADS_TAG", "Vungle: ResumeAd : onAdFailedToLoad: $adError")
                    onAdFailed?.invoke()
                    currentActivity?.let {
                        if (BuildConfig.DEBUG) {
                ...
                override fun onAdFailedToPlay(baseAd: BaseAd, adError: com.vungle.ads.VungleError) {
                    Log.i("DP_ADS_TAG", "Vungle: ResumeAd : onAdFailedToPlay: $adError")
                    onAdFailed?.invoke()
                    dismissWaitDialog()
                    currentActivity?.let {
                        if (BuildConfig.DEBUG) {
                ...
                override fun onAdClicked(baseAd: BaseAd) {
                    Log.i("DP_ADS_TAG", "Vungle: ResumeAd : onAdClicked()")
                }

                override fun onAdLeftApplication(baseAd: BaseAd) {
                    Log.i("DP_ADS_TAG", "Vungle: ResumeAd : onAdLeftApplication()")
                }

                override fun onAdImpression(baseAd: BaseAd) {
                    Log.i("DP_ADS_TAG", "Vungle: ResumeAd : onAdImpression()")
                }
            }
            load()
        }
    ...
    // Dialog showing while ad is loading
    ...
    // Dismiss dialog once ad is loaded
